...
const ronModelPath = '/models/ron_weasley.glb';

export function RonWeasleyModel(props) {
  useEffect(() => {
    console.log('RonWeasleyModel component mounted in R3F scene.');
  }, []);

  const { nodes, materials } = useGLTF(ronModelPath);

  // Log nodes and materials to confirm exact names from gltfjsx output
  console.log('Ron Weasley Model loaded. Nodes:', nodes, 'Materials:', materials);
  ...
  if (!nodes || Object.keys(nodes).length === 0 || !materials || Object.keys(materials).length === 0) {
    console.warn("WARNING: Ron model loaded but no nodes or materials found. Rendering a placeholder box.");
    return (
      <mesh {...props}>

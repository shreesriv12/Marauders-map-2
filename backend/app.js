...
// --- Marauder's Map Specific In-Memory Storage ---
const activeUsers = {}; // Stores { userId: { latitude, longitude, lastUpdate, isTrackingActive, socketId, name } }
...
io.on('connection', (socket) => {
    ...
    // Frontend sends: { userId: 'some_uuid_or_username' }
    socket.on('registerUser', (data) => {
        userId = data.userId; // Extract userId from the data object
        ...
        if (!activeUsers[userId]) {
            activeUsers[userId] = {
                ...
                name: data.name || userId // Store name if provided, else use userId
            };
        } else {
            activeUsers[userId].socketId = socket.id; // Update socket ID if user reconnects
            activeUsers[userId].name = data.name || activeUsers[userId].name; // Update name if provided
        }

        // Send the current map activation status to the newly connected user
        // Use io.to(socket.id) to send only to this specific client
        io.to(socket.id).emit('mapActivationStatus', activeUsers[userId].isTrackingActive);
        console.log(`[RegisterUser] Sent mapActivationStatus (${activeUsers[userId].isTrackingActive}) to ${userId}`);
            ...
            for (const id in activeUsers) {
                ...
                if (activeUsers[id].isTrackingActive && activeUsers[id].latitude !== null && activeUsers[id].longitude !== null) {
                    usersToSend[id] = {
                        ...
                        name: activeUsers[id].name // Include name
                    };
                }
            }
    ...
    // Frontend sends: { userId, latitude, longitude, name }
    socket.on('sendLocation', (data) => {
        ...
        if (data.userId && activeUsers[data.userId]) { // Use data.userId directly
            // Always update user's location in memory
            activeUsers[data.userId].latitude = data.latitude;
            activeUsers[data.userId].longitude = data.longitude;
            activeUsers[data.userId].lastUpdate = Date.now();
            activeUsers[data.userId].name = data.name || data.userId; // Update name

            // Only broadcast the updated location to all other connected clients IF they are tracking
            if (activeUsers[data.userId].isTrackingActive) {
                // socket.broadcast.emit sends to all clients except the sender
                socket.broadcast.emit('locationUpdate', {
                    userId: data.userId,
                    latitude: data.latitude,
                    longitude: data.longitude,
                    name: data.name // Include name in broadcast
                });
                console.log(`[SendLocation] Broadcasting location update for ${data.userId}`);
            } else {
                console.log(`[SendLocation] User ${data.userId} is not active, not broadcasting location.`);
            }
        } else {
            console.warn(`[SendLocation] Attempted to send location for unregistered or invalid userId: ${data.userId}`);
        }
    });

    // Event listener for map activation spell (Marauder's Map)
    // Frontend sends: { userId: 'some_uuid_or_username' }
    socket.on('activateMap', (data) => {
        const activatingUserId = data.userId; // Extract userId from data object
        if (activeUsers[activatingUserId]) {
            ...
            if (activeUsers[activatingUserId].latitude !== null && activeUsers[activatingUserId].longitude !== null) {
                socket.broadcast.emit('locationUpdate', {
                    ...
                    name: activeUsers[activatingUserId].name // Include name
                });
                console.log(`[ActivateMap] Broadcasting initial location for newly active user ${activatingUserId}`);
            }
        } else {
            console.warn(`[ActivateMap] Attempted to activate map for unregistered or invalid userId: ${activatingUserId}`);
        }
    });

    // Event listener for "Mischief Managed" (deactivation) (Marauder's Map)
    // Frontend sends: { userId: 'some_uuid_or_username' }
    socket.on('deactivateMap', (data) => {
        const deactivatingUserId = data.userId; // Extract userId from data object
        if (activeUsers[deactivatingUserId]) {
            ...
            // Changed from 'userDisconnected' to 'userLeft' to match frontend
            socket.broadcast.emit('userLeft', { userId: deactivatingUserId }); // Send as object
            console.log(`[DeactivateMap] Broadcasted userLeft for ${deactivatingUserId}`);
        } else {
            console.warn(`[DeactivateMap] Attempted to deactivate map for unregistered or invalid userId: ${deactivatingUserId}`);
        }
    });

    // Event listener for requesting active users (e.g., when map becomes active) (Marauder's Map)
    // Frontend sends: { userId: 'some_uuid_or_username' }
    socket.on('requestActiveUsers', (data) => {
        const requestingUserId = data.userId; // Extract userId from data object
        console.log(`[RequestActiveUsers] Socket ${socket.id} (User ID: ${requestingUserId}) requested active users.`);
        const usersToSend = {};
        for (const id in activeUsers) {
            // Only send locations of users who are actively tracking and have valid coordinates
            if (activeUsers[id].isTrackingActive && activeUsers[id].latitude !== null && activeUsers[id].longitude !== null) {
    ...
    socket.on('disconnect', () => {
        ...
        if (userId && activeUsers[userId]) { // Use the 'userId' closure variable
            ...
            // Changed from 'userDisconnected' to 'userLeft' and send as object
            io.emit('userLeft', { userId: userId });
            console.log(`[Disconnect] Broadcasted userLeft for ${userId}`);
